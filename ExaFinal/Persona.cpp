...
string Persona::generarpasword(){
    
    char inicialF = tolower(apellido[apellido.length()-1]);




    char inicial1 = tolower(nombre1[0]);
    char inicial2 = tolower(nombre2[nombre2.length()-1]);
    string paswordmix=apellidorev(apellido);
    std::transform(paswordmix.begin(), paswordmix.end(), paswordmix.begin(), ::tolower);
    string pasword = string(1,inicialF) + string(1, inicial1) + string(1, inicial2) + paswordmix;
    string paswordDef = pasword;
    return paswordDef;
}

string  Persona::generarencriptado(string texto){
...
string Persona::generarID() {
    // Generar el ID automáticamente
    static int contador = 1;
    string id = "ESPE-" + std::to_string(contador);
    contador++;

    // Guardar el nuevo ID en BaseDatosIDs.txt
    std::ofstream archivoSalidaIDs("BaseDatosIDs.txt", std::ios::app);
    if (archivoSalidaIDs.is_open()) {
        archivoSalidaIDs << id << std::endl;
        archivoSalidaIDs.close();
    }
